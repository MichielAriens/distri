 ...
 */
public abstract class AbstractScriptedSimpleTripTest<ReservationSession, ManagerSession> {
    ...
    public AbstractScriptedSimpleTripTest(String scriptFile) {
        this.scriptFile = scriptFile;
    }

    public void run() throws Exception {
        //read script from JAR
        BufferedReader in = new BufferedReader(new InputStreamReader(AbstractScriptedSimpleTripTest.class.getClassLoader().getResourceAsStream(scriptFile)));
        int lnr = 0;
        //while we have lines
        while (in.ready()) {
            lnr++;
